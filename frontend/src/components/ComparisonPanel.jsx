...
const ComparisonPanel = ({
  ...
  const insights = useMemo(() => {
    ...
    const insightsList = [];
    const sortedByPrice = [...enhancedProducts].sort(
      (a, b) => a.price - b.price
    );
    const sortedByRating = [...enhancedProducts].sort(
        ...
        <div className="overflow-y-auto overflow-x-auto p-4 md:p-6 custom-scrollbar">
          ...
          {compareList.length > 1 && (
            <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              {insights.map((insight, idx) => (
                <div
                  key={idx}
                  className="bg-gradient-to-br from-gray-800 to-gray-800/50 p-4 rounded-xl border border-gray-700 shadow-lg relative overflow-hidden group hover:border-cyan-500/30 transition-all"
                >
          ...
          <div className="grid grid-cols-[120px_repeat(auto-fit,minmax(200px,1fr))] gap-0 divide-x divide-gray-700 border border-gray-700 rounded-2xl bg-gray-900/50 overflow-hidden">
            ...
            <div className="bg-gray-800/30 flex flex-col">
              <div className="h-48 p-4 flex items-end font-semibold text-gray-400 text-sm border-b border-gray-700">
                Product
              </div>
              <div className="h-14 p-4 flex items-center font-semibold text-gray-400 text-sm border-b border-gray-700">
                Price
              </div>
              <div className="h-14 p-4 flex items-center font-semibold text-gray-400 text-sm border-b border-gray-700">
                Rating
              </div>
              <div className="h-32 p-4 flex items-start pt-4 font-semibold text-gray-400 text-sm border-b border-gray-700">
                Features
              </div>
              <div className="h-14 p-4 flex items-center font-semibold text-gray-400 text-sm">
                Action
              </div>
            </div>
            ...
            {enhancedProducts.map((item) => (
              <div
                key={item._id}
                className="flex flex-col min-w-[200px] relative group hover:bg-gray-800/30 transition-colors"
              >
                <button
                  type="button"
            ...
            {[...Array(Math.max(0, 2 - compareList.length))].map((_, i) => (
              <div
                key={`empty-${i}`}
                className="hidden md:flex flex-col min-w-[200px] bg-gray-900/20"
              >
